...
export default function EmployerAppointments() {
  ...
  const fetchAppointments = (currentFilter, currentDateVal) => {
    setLoading(true);
    let url = `/api/employer/appointments?filter=${currentFilter}`;
    ...
    axios.get(url)
      ...
      .catch((err) => console.error('Failed to load employer appointments', err))
    ...
    <section className="max-w-7xl mx-auto px-4 py-10 text-left fade-in">
      <div className="grid lg:grid-cols-[260px_1fr] gap-6">
        ...
        <div>
          ...
          <p className="text-slate-600 mb-6">Track and manage scheduled patient consultation sessions with doctors.</p>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {appointments.map((a) => (
                      <tr key={a.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="p-4 text-slate-700 font-medium">{a.doctor_name}</td>
